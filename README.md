# EHR Harness Plugin — Year-End Edition

> Based on [qoxmfaktmxj/ehr-harness-plugin](https://github.com/qoxmfaktmxj/ehr-harness-plugin). 연말정산(Year-End Tax Settlement) 도메인 보강 버전. 자세한 내용은 [NOTICE.md](./NOTICE.md) 참고.
...
## 이 레포에 대해 (Year-End Edition)

이 레포는 [qoxmfaktmxj/ehr-harness-plugin](https://github.com/qoxmfaktmxj/ehr-harness-plugin) 의 fork 로,
원본 플러그인 위에 **연말정산(yjungsan) 도메인 전용 패키지**를 추가로 묶은 버전이다.

### 무엇을 위해

원본 `ehr-harness` 는 EHR4/5 프로젝트에 **범용** 하네스를 자동 생성한다 (화면·프로시저·DB·릴리즈 검증 등).
하지만 연말정산 업무는 도메인 특수성이 강하다.

- 22개 테이블(`TCPN8##` 계열)과 6개 패키지(`PKG_CPN_YEA_*`)가 5개 레이어로 얽혀 동작
- 비즈니스 로직 대부분이 Java 보다 Oracle 패키지 본문에 집중 (2026 기준 본문만 수만 라인)
- 마감 체인(`P_CPN_YEA_CLOSE`)이 별도 상태 기계로 작동
- 매년 개정세법 반영, 후속조치 hotfix, 특수 케이스 대응이 반복

이런 도메인 지식을 **별도 플러그인**(`ehr-yearend-harness`)으로 패키징해서 원본 위에 **추가 설치**할 수 있게 한 것이 본 레포의 목적이다.

### 원본 출처

- **원본**: [qoxmfaktmxj/ehr-harness-plugin](https://github.com/qoxmfaktmxj/ehr-harness-plugin)
- **원 저작자**: qoxmfaktmxj
- 원본 저장소에 LICENSE 가 명시되지 않아, 추후 원 저작자 방침 확인 후 갱신. 자세한 출처·라이선스 표기는 [NOTICE.md](./NOTICE.md) 참고.
...
`ehr-yearend-harness` 의 자세한 내부 구조와 발동 시점은 [`plugins/ehr-yearend-harness/README.md`](./plugins/ehr-yearend-harness/README.md) 를 참고.

### 설치 방법은?

아래 [2. 플러그인 설치 방법](#2-플러그인-설치-방법) 섹션의 **방법 3** 에 본 fork 의 두 플러그인 모두 설치하는 절차가 정리되어 있다.
