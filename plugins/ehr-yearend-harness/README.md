...
Codex 는 repo-local marketplace 를 읽을 수 있으므로, 이 저장소를 marketplace root 로 추가한 뒤
...
| Codex 에서 플러그인이 안 보임 | repo-local marketplace 를 쓰려면 `.agents/plugins/marketplace.json` 이 필요하다. 이 저장소에는 포함되어 있으므로 marketplace 추가 후 Codex 를 재시작한다. |
