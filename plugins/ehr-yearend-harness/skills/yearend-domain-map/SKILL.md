...
5. 질의에 `세액`, `가산세`, `산출과표`, `과세급여`, `사업소세`, `종업원분`, `납부지연`, `감면`, `절사` 가 포함되면 `../../references/yjungsan-tax-calc-rules.md` 를 **반드시** 읽는다. 특히 용어 정의(§1)를 먼저 확인하여 이중 차감 등 오계산을 방지한다.
6. **참조 문서에 없는 정보는 "참조 문서에 없음" 이라고 명시**한다. 추측하지 않는다.
7. 연도 의존 객체(`PKG_CPN_YEA_{YY}_*`)를 답변할 때는 귀속연도를 명시하거나 사용자에게 확인한다.
...
- `../../references/yjungsan-tax-calc-rules.md` — 세액 계산 규칙 (용어 정의·가산세·절사)
