...
description: 연말정산 실행 체인을 양방향으로 추적하고, 도메인 DB 조회 쿼리 작성·영향 분석·테스트 데이터 위치 안내·고객/회사별 분기 보조 확인까지 제공하는 스킬. 화면(BefComMgr, YearIncomeMgr 등) · 테이블(TCPN8##) · 프로시저(P_CPN_YEA_CLOSE, PKG_CPN_YEA_{YY}_*) · 증상(총급여 0원 나옴 등) · 고객사/그룹명 중 어느 것을 입력으로 받아도 연관 코드 경로·DB 객체·마감 영향을 뽑아낸다. "이 화면 건드리면", "이 테이블 변경 시", "마감에 영향 가나", "체인 추적", "영향 범위", "조회 쿼리 짜줘", "이 데이터 확인", "테스트 데이터 어디 있어", "고객별 분기", "고객사별", "회사별", "선배포", "커스텀", "사이트별 예외" 요청에 사용.
---
...
체인 추적 입력에 다음 키워드가 포함되면 `references/yjungsan-customer-variants.md` 를 보조 reference 로 확인한다.

- 명시적 키워드: `고객별 분기`, `고객사별`, `회사별`, `선배포`, `커스텀`, `사이트별 예외`
- `references/yjungsan-customer-variants.md` 에 등록된 고객 식별자 (예: `고객A`, `고객B`, …)
...
|---|---|
| 키워드 미감지 | 8번 섹션 생략 |
| 키워드 감지 + 매칭 있음 | 8번 섹션 정식 출력 (감지 키워드 / 매칭 / 확인 위치 / 판정) |
| 키워드 감지 + 매칭 없음 | 8번 섹션 한 줄 출력 ("매칭 없음 — 추측 안 함"). v0.3.0 출시 직후엔 `customer-variants.md` 가 빈 슬롯이라 이 분기가 기본 |

**금지 표현**: "고객X 는 별도 로직 적용됨" 식 단정. 항상 "확인 필요" 표시 + 위치 인용.

---

## Binary 파일 fail-fast 정책 (v0.3.1)

다음 파일은 *내부 검사 시도 1회 후 즉시 외부 도구 필요로 표시*하고 다음 단계로 넘어간다.

- `*.mrd` (M2soft RD 바이너리, gzip 압축)
- `*.pdf` (PDF 바이너리)
- `*.zip`, `*.jar`, `*.class` (압축/컴파일)
- 첫 1KB 안에 non-printable 바이트 비율이 30% 이상인 임의 파일

표시 형식 (출력 또는 영향 분석 표 안):

> `<파일명>` — 외부 도구 필요 (M2soft RD 디자이너 / PDF 뷰어 / `unzip` 등)

**금지 동작**: `strings` / hex dump / `iconv` / null byte 제거 / 다단계 추출 시도. 시간 낭비 + 결국 실패. 1회 시도 후 즉시 표시.

이 정책은 `yearend-investigator` 의 *Step 0 시간 가드* 와 연동된다.
...
항상 다음 섹션으로 답한다. 6·7·8번은 조건부 — 해당 조건이 충족될 때만 채우고, 아니면 섹션 자체 생략.
...
8. **고객/회사별 분기 가능성** (감지 시) — 입력에 고객/회사 키워드가 감지된 경우만. 매칭 있음일 때 정식 4줄 출력, 매칭 없음일 때 한 줄 출력. 미감지 시 섹션 자체 생략.

### 8번 섹션 형식 (감지 + 매칭 있음)

```
8. **고객/회사별 분기 가능성** (감지 시)
   - 감지 키워드: <키워드>
   - `references/yjungsan-customer-variants.md` 매칭: <행 인용>
   - 확인 필요 위치: <파일:line 또는 패키지/매퍼 ID>
   - 판정: 고객/회사별 분기 가능성 있음 — 확인 필요
```
