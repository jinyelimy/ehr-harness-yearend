...
> "**연말정산 도메인 전용 지식(참조 3) + 자동 스킬 2개 + 조립 에이전트 1개 + 타깃 연결 스크립트 2개**. 실제 EHR_HR50 프로젝트에 정션으로 연결하면 Claude Code 가 연말정산 질문에 즉시 반응한다."
...
## 5. 설치 (타깃 프로젝트에 연결하는 법)

이 하네스를 실제로 쓰려면 **타깃 EHR 프로젝트의 `.claude/`** 에 정션으로 연결해야 한다.
편집은 이 레포에서만 하고, 타깃에서는 정션을 통해 같은 파일을 본다.
...
결과: 타깃의 `.claude/skills/` 아래에 `yearend-domain-map`, `yearend-chain-tracer` 디렉터리 정션이
생기고, `.claude/agents/yearend-investigator.md` 하드링크가 생긴다.

### 5.2 복사 방식 (정션 불가 환경용 백업)

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/sync-to-target.ps1 `
    -SourceRoot   "C:\yelingg\ehr-harness-yearend\drafts\yearend-harness" `
    -TargetClaudeDir "C:\Users\jinyelimy\isu-hr\EHR_HR50\.claude"
```

`robocopy /MIR` 유사 동작. 소스에서 삭제된 파일은 타깃(단 `yearend-*` 접두어만)에서도 제거.
...
`yearend-domain-map` 스킬이 자동 호출돼 references 기반 답변이 나오면 연결 정상.

---

## 6. 범위 제외 (Out of Scope)

- 플러그인(Claude Code Plugin) 패키징. 프로토타입 검증 후 별도 결정.
